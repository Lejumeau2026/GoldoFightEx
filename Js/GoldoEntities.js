...
Entity.prototype.ForceHit = function () {
	this.vie--;
...
Entity.prototype.ApplyEffect = function (type) {
};


Entity.prototype.IsEndPhase = function () {
	return false;
};

Entity.prototype.StopGame = function () {
};
...
function Goldorak() {
	...
	this.ModelGoldoBrasFulguropoing = null;

	this.FulguroPoing01 = null;
	this.FulguroPoing02 = null;

	this.fireFreezeFx = null;
	...
	this.MissilPlanitron = function () {
		...
		if (this.fireFreezeFx.IsActive()) return;
	...
	this.Reset = function () {
		...
		// this.MegaMach.nbAccumulate = 0;
		this.MegaMach.Reset();
		
		this.FulguroPoing01.Reset();
		this.FulguroPoing02.Reset();
		this.fireFreezeFx.Reset();
	}
	...
	// BonusMortanium
	// AppViewModel.bonusMortanium
	this.WinMortanium = function () {
		
		if (this.Alcorak != null)
		{
			loader.PlaySound('Mortanium', 1);
			this.Alcorak.WinMortanium();
		}
		// Si il n'y a pas d'Alcorak on gagne les points du bonus mais c'est tout
		else loader.PlaySound('Bonus', 1);
	};
	...
	this.NewMegaMach = function () {
		...
		scrollZone.IncSpeedTarget(800, 400);
		
		this.MegaMach.ResetTimeLife();
		this.MegaMach.Accumulate();
	}
	...
	this.DrawFulguropoingMode = function (ctx) {
		...
		ctx.drawImage(this.ModelGoldoBrasFulguropoing.img, this.posx + 78, this.posy);
		ctx.drawImage(this.ModelGoldoPoing.img, this.posx, this.posy);
...
Goldorak.prototype.Init = function () {
	...
	this.ModelGoldoBrasFulguropoing = loader.GetModel('GoldoBrasFulguropoing');
	...
	this.fireFreezeFx = new FireFreezeFx();
	this.fireFreezeFx.Init();	
};
...
Goldorak.prototype.Animate = function (deltaTime) {
	...
	this.fireFreezeFx.Animate (deltaTime);
...
Goldorak.prototype.Collide = function (entity) {
	...
	if (entity.CanCollide() == false) return false;
...
// Pour l'instant type n'est pas utilisé il n'y a qu'un seul effet
Goldorak.prototype.ApplyEffect = function (type) {
	this.fireFreezeFx.ActiveFx();
};


function FireFreezeFx () {

	this.timeFireFreeze = new Timer(randomUniform(2,5)); // de 2 à 4 seconde de freeze
	this.timeFireFreezeEffect = new Timer(0.2);
	this.freezeImage = new Indexer(1, 3);
	
	this.Enable = false;
	this.EnergyFreezeMod = null;
	
	this.ActiveFx = function () {
		loader.PlaySound('PowerDown', 1);
		this.Enable = true;
		this.timeFireFreezeEffect.Reset();
		this.timeFireFreeze.Reset();
	}
...
FireFreezeFx.prototype = Object.create(Entity.prototype);
FireFreezeFx.prototype.constructor = FireFreezeFx;

FireFreezeFx.prototype.Animate = function (deltaTime) {

	if (this.Enable == false) return;

	this.timeFireFreezeEffect.Process(deltaTime);
	if (this.timeFireFreezeEffect.IsOver())
	{
		this.timeFireFreezeEffect.Reset();
		this.freezeImage.Inc();

	}
	
	this.timeFireFreeze.Process(deltaTime);
	if (this.timeFireFreeze.IsOver())
	{
		this.Enable = false;
		this.timeFireFreeze.Reset();
	}
...
FireFreezeFx.prototype.Init = function () {
	this.EnergyFreezeMod = new Array();

	for (var i = this.freezeImage.minValue; i <= this.freezeImage.maxValue; i++) {
		this.EnergyFreezeMod[i] = loader.GetModel('EnergieFreezeFireMap' + i);
	}
};

FireFreezeFx.prototype.Draw = function (ctx) {
	if (this.Enable == false) return;
	ctx.drawImage(this.EnergyFreezeMod[this.freezeImage.curValue].img, Goldo.posx, Goldo.posy);
}
...
function CyberMine() {
	...
	this.phase = 1;
	this.phaseTime = 0;
	this.modelDrawTime = 0;
	this.AttachedRelPosx = 0; // Rel comme relative
	this.AttachedRelPosy = 0;
	this.AttachedPhaseTimeMax = randomUniform (2,6);
	this.modelFlash = null;
	this.modelDraw = null;
	
	
	// La mine traverse l'écran et est
	// attirée par Goldorak par le magnétisme
	this.AnimatePhaseFly = function (deltaTime) {
	...
	// Animation de la mine quand elle est collée à Goldorak
	this.AnimatePhaseAttached = function (deltaTime) {

		this.phaseTime += deltaTime;
		this.modelDrawTime += deltaTime;
		if (this.modelDrawTime > 0.3)
		{
			this.modelDrawTime = 0;
			this.modelDraw = (this.modelDraw == this.model) ? this.modelFlash : this.model;
		}
		
			
		
		if (this.phaseTime > this.AttachedPhaseTimeMax) 
		{
			this.phase = 3;
			// Sequencer.AddAttachedCybermine(-1);
		}
	}
	
	
	// Le temps limite est atteint elle explose
	this.AnimatePhaseExplose = function (deltaTime) {
		
		// Ne pas passer par la method Hit car elle vérifie si il est invulnérable
		// et dans le cas d'une cybermine il ne l'est pas
		Goldo.ForceHit();
		
		this.SetNoValid();

		var epx = Goldo.posx + Goldo.model.img.width / 2 + this.AttachedRelPosx;
		var epy = Goldo.posy + Goldo.model.img.height / 2 + this.AttachedRelPosy;
		NewCrosseExplosionPos(epx,epy);
		
	}
	
	this.AttachToGoldo = function (deltaTime) {

		var adx = (this.posx + this.model.img.width / 2) - (Goldo.posx + Goldo.model.img.width / 2);
		var ady = (this.posy + this.model.img.height / 2) - (Goldo.posy + Goldo.model.img.height / 2);
		var len = Math.sqrt(adx*adx + ady*ady);
		adx /= len;
		ady /= len;
		
		this.ang = Math.atan2(ady, adx) + Math.PI / 2;
		this.AttachedRelPosx = adx * (Goldo.radius * 1.5);
		this.AttachedRelPosy = ady * (Goldo.radius * 1.5);
		this.phase = 2;
		Sequencer.AddAttachedCybermine(1);
	}
...
CyberMine.prototype.Init = function () {
	...
	this.modelDraw = this.model;
	
	this.modelFlash = loader.GetModel('CyberMineFlash')
...
CyberMine.prototype.Animate = function (deltaTime) {
	...
	switch (this.phase)
	{
		case 1:this.AnimatePhaseFly(deltaTime);break;
		case 2:this.AnimatePhaseAttached(deltaTime);break;
		default:this.AnimatePhaseExplose(deltaTime);break;
	}
};


CyberMine.prototype.IsInvulnerable = function () {
	return (this.phase > 1) ? true : false;
};
...
CyberMine.prototype.Draw = function (ctx) {
	...
	{
		var tx = Goldo.posx + Goldo.model.img.width / 2 + this.AttachedRelPosx;
		var ty = Goldo.posy + Goldo.model.img.height / 2 + this.AttachedRelPosy;
		// Juste pour le debug, trace le centre de la mine et l'axe directeur
		// DrawCircle (ctx,tx,ty, 4);
		// DrawLine (ctx,Goldo.posx + Goldo.model.img.width / 2,Goldo.posy + Goldo.model.img.height / 2,Goldo.posx + Goldo.model.img.width / 2 + this.AttachedRelPosx,Goldo.posy + Goldo.model.img.height / 2 + this.AttachedRelPosy,'#AAAAFF');
		ctx.translate(tx,ty);
		ctx.rotate(this.ang);
		
		ctx.drawImage(this.modelDraw.img, -this.modelDraw.img.width / 2, -this.modelDraw.img.height / 2);
		// ctx.drawImage(this.modelFlash.img, -this.modelFlash.img.width / 2, -this.modelFlash.img.height / 2);
		// ctx.drawImage(this.model.img, -this.model.img.width / 2, -this.model.img.height / 2);
	}
...
CyberMine.prototype.CanCollide = function () {
	return false;
};
...
CyberMine.prototype.Collide = function (entity) {
	...
	// Si on n'est pas en phase 1 alors cela signifie que la mine est collée à Goldorak
	if (this.phase > 1) return false;

	if (entity.model.isBonus == true) return false;
	
	var radiusColision = this.radius;
	
	// Pour un collision par magnétisme on divuse le rayon par deux
	// pour donner plus de chance aux entités ci-dessous
	if (entity == Goldo || entity == Goldo.Alcorak || entity == Goldo.Fossoirak || entity == Goldo.Venusiak || entity == Goldo.OVTerre)
	{
		radiusColision /= 2;
	}
	
	// On fait le test de collision
	if (CollideCircle(this, radiusColision, entity, entity.radius) == false) return false;

	// Si la collision se fait avec Goldorak alors on colle la mine et on passe en phase deux
	if (entity == Goldo)
	{
		this.AttachToGoldo();
		return false;
	...
	entity.Hit();
	if (this.IsInvulnerable() == false) this.Hit();

	if (entity == Goldo.Alcorak || entity == Goldo.Fossoirak || entity == Goldo.Venusiak || entity == Goldo.OVTerre)
	{
		// Ici on initialise le curseur de vie à 0 pour que l'extra bonus puisse s'activer
		// quand on a détruit toutes les mines. On admet, pour etre gentil, qu'une collision*
		// avec un des aeronef compte 
		this.vie = 0;
	}
...
CyberMine.prototype.OnEnded = function () {
	if (this.vie <= 0) {
		...
		Sequencer.CheckCyberMineBonus();
	}
	
	if (this.phase > 1) Sequencer.AddAttachedCybermine(-1);
};
...
function GrandeNavette() {
	...
	this.GrandeNavetteMap = new Array();
	this.RotateTime = 0;
	this.iCurentImgDraw = 1;
}
...
GrandeNavette.prototype.Init = function () {
	...
	this.GrandeNavetteMap = new Array();
	for (var i = 1; i <= 6; i++) {
		this.GrandeNavetteMap[i] = loader.GetModel('GrandeNavetteMap' + i);
	}

	// Fixer le modèle sur la première image
	this.SetModel(this.GrandeNavetteMap[this.iCurentImgDraw]);
...
GrandeNavette.prototype.Animate = function (deltaTime) {

	Navette.prototype.Animate.call(this, deltaTime);

	this.RotateTime += deltaTime;
	if (this.RotateTime > 0.075) // 0.05
	{
		this.RotateTime = 0;
		this.iCurentImgDraw ++;
		if (this.iCurentImgDraw > 6) this.iCurentImgDraw = 1;
		this.model = this.GrandeNavetteMap[this.iCurentImgDraw];
	}
};
...
function MissilRampe(ParentEntity, relposx, relposy, defaultDir) {
	...
	this.preferedMissilSpeed = 0;
	this.preferedFireTimeIteration = 0;
	...
	this.Fire = function () {
		...
		missil.SetModel(missilModel);
		missil.speed = (this.preferedMissilSpeed != 0) ? this.preferedMissilSpeed : missilModel.speed;
	...
	this.SetPreferedParameter = function (preferedMissilSpeed,preferedFireTimeIteration) {
		this.preferedMissilSpeed = preferedMissilSpeed;
		this.preferedFireTimeIteration = preferedFireTimeIteration;
	}
...
MissilRampe.prototype.Animate = function (deltaTime) {
	...
	if (this.lastFireTime > this.FireTimeIteration) {
		this.lastFireTime = 0;
		// this.FireTimeIteration = randomUniform(1000, 4000) / 1000;
		this.FireTimeIteration = (this.preferedFireTimeIteration != 0) ? this.preferedFireTimeIteration : randomUniform(1000, 4000) / 1000;
		this.Fire();
	}
};
...
FulguroPoing.prototype.Collide = function (entity) {
	...
	// Que l'entité soit encore en vie ou non
	// on réalise une explosion pour montrer l'impacte
	NewExplosion(this, entity);
...
function Bonus(BonusName) {
	...
	this.WinPlanitron = function () {
		// loader.PlaySound('Bonus', 0.15);
		loader.PlaySound('PlanitronBonus', 1);
	...
	this.WinBonusAmirale3D = function () {
		loader.PlaySound('Bonus', 0.15);
		AppViewModel.hasBonusAmirale3D = true;
	};
}
...
Bonus.prototype.Init = function () {
	...
	switch (this.BonusName) {
		...
		case 'BonusMortanium':
			this.processFx = this.WinMortanium;
			break;
		...
		case 'BonusAmirale3D':
			this.processFx = this.WinBonusAmirale3D;
			break;
...
Lazernium.prototype.PlaySound = function () {
	...
	loader.PlaySound('Lazernium', 1); // 0.5
};
...
function GinginLazernium(ParentEntity, LazerName) {
	...
	this.FireAnimate = function (deltaTime) {
		if (this.InFire == false) {
			...
			if (this.fireTimeSpent > this.fireDuration) {
				...
				{
					this.tmpLazerHeight = this.model.img.height;
					if (this.endFireOnDistance == true)
					{
						this.InFire = false;
						this.lastFireTime = Date.now();
						this.nextFireTime = randomUniform(1, 6) * 1000;
	...
	this.SetRandomFireTime = function () {
		this.InFire = false;
		this.lastFireTime = Date.now();
		this.nextFireTime = randomUniform(1, 6) * 1000;
	}	
...
function SoucoupeAmirale3D() {

	Entity.call(this);
	this.phase = 1;

	this.timePhase = new Timer(10);
	
	this.vie = 1000000; // vie Illimité
	this.valid = true;
	this.rampeEnabled = false;
	this.lazerniumEnabled = false;
	
	this.missilRampes = null;
	this.rayonEnergy = null;
	this.sortieNavette = null;
	this.lazerniums = null;
	...
	this.EnableSortieNavette = function (enable) {
		this.sortieNavette.Enable = enable;
	};

	this.EnableRampe = function (enable) {
		this.rampeEnabled = enable;
	};
	
	this.EnableLazernium = function (enable) {
		this.lazerniumEnabled = enable;
	};

	this.EnableEnergy = function (enable) {

		if (enable == false)
		{
			if(this.rayonEnergy.InFire) this.rayonEnergy.NewMissil();
			this.rayonEnergy.ResetTiming ();
		}
		
		this.rayonEnergy.Enable = enable;
	};
	

	this.SetPhase = function (phase,anims,timemax) {
		this.phase = phase;
		
		if (timemax != undefined) this.timePhase.SetTimeMax (timemax, true);
		else this.timePhase.Reset();
			
		this.EnableSortieNavette ((anims & Amirale3dAnim.SortieNavette) != 0);
		this.EnableEnergy ((anims & Amirale3dAnim.Energy) != 0);
		this.EnableRampe ((anims & Amirale3dAnim.Rampe) != 0);
		this.EnableLazernium ((anims & Amirale3dAnim.Lazernium) != 0);
	};


	this.GetHolePos = function (hole) {

		var relposx = 0;
		var relposy = 0;
		switch (hole) {
			case 1: relposx = 324; relposy = 893; break;
			case 2: relposx = 720; relposy = 893; break;
			case 3: relposx = 400; relposy = 928; break;
			case 4: relposx = 643; relposy = 928; break;
			case 5: relposx = 482; relposy = 940; break;
			case 6: relposx = 562; relposy = 940; break;

			case 7: relposx = 324; relposy = 150; break;
			case 8: relposx = 720; relposy = 150; break;
			case 9: relposx = 400; relposy = 115; break;
			case 10: relposx = 643; relposy = 115; break;
			case 11: relposx = 482; relposy = 100; break;
			case 12: relposx = 562; relposy = 100; break;
		}

		return new Vector(relposx, relposy);
...
SoucoupeAmirale3D.prototype = Object.create(Entity.prototype);
SoucoupeAmirale3D.prototype.constructor = SoucoupeAmirale3D;

SoucoupeAmirale3D.prototype.Init = function () {
	this.RandomStartPosition();
	
	this.timePhase.Reset();
	
	this.missilRampes = new Array();
	for (var i = 1; i <= 6; i++) {
		
		var relpos = this.GetHolePos(i);
		var rampe = new MissilRampe(this, relpos.x, relpos.y, new Vector(0, 1));
		// 600 = preferedMissilSpeed
		// 0.8 = preferedFireTimeIteration
		rampe.SetPreferedParameter (1200,0.7);
		this.missilRampes.push(rampe);
	}
	...
	for (var i = 1; i <= 12; i++) {
		...
		var lazer = new LazerRampe(this, relpos.x, relpos.y, new Vector(0, (i < 7) ? 1 : -1));
		lazer.Init();
		this.lazerniums.push(lazer);
	}

	this.rayonEnergy = new RayonEnergy(this, this.model.img.width/2, this.model.img.height/2, 'RayonEnergie');
	this.rayonEnergy.Init();

	this.sortieNavette = new SortieNavette (this);

};



SoucoupeAmirale3D.prototype.Animate = function (deltaTime) {

	if (this.valid == false) return;

	this.timePhase.Process (deltaTime);

	switch (this.phase)
	{
		case 1: // On descend
			this.posy += this.dy * this.speed * deltaTime;
			if (this.posy >= -this.model.img.height*(3/4)+25) 
			{
				this.SetPhase (2, Amirale3dAnim.SortieNavette, 15);
			}
			break;

		case 2:
			if (this.timePhase.IsOver())
			{
				// this.SetPhase (3, Amirale3dAnim.Rampe | Amirale3dAnim.Energy, 15);
				this.SetPhase (3, Amirale3dAnim.Lazernium, 15);
			}
			break;
		...
		case 3:
			if (this.timePhase.IsOver())
			{
				this.SetPhase (4, Amirale3dAnim.SortieNavette | Amirale3dAnim.Energy, 15);
				
			}

			this.posy += this.speed * deltaTime;
			if(this.posy > AppViewModel.canvasize.height) this.posy = -this.model.img.height;
			break;
		...
		case 4:
			if (this.timePhase.IsOver())
			{
				this.SetPhase (5, Amirale3dAnim.Rampe, 15);
			}
		...
		case 5:
			if (this.timePhase.IsOver())
			{
				this.SetPhase (6, Amirale3dAnim.SortieNavette | Amirale3dAnim.Lazernium | Amirale3dAnim.Energy);
			}
			break;

		case 6: // Retour au bercaille
			this.posy -= this.speed * deltaTime;
			if (this.posy < -this.model.img.height-50) 
			{
				this.SetPhase (7, Amirale3dAnim.None);
				this.SetNoValid();
			}
			break;
	}

	if (this.posx + this.model.img.width / 2 > Goldo.posx + Goldo.model.img.width / 2) this.dx = -1;
	else if (this.posx + this.model.img.width / 2 < Goldo.posx + Goldo.model.img.width / 2) this.dx = 1;
	else this.dx = 0;
	
	this.posx += this.dx * this.speed * deltaTime;

	if (this.sortieNavette != null && this.sortieNavette.Enable == true)
		this.sortieNavette.Animate(deltaTime);

	this.rayonEnergy.Animate(deltaTime);
	
	if (this.missilRampes != null && this.rampeEnabled == true)
	{
		for (var i = 0; i < this.missilRampes.length; i++) {
			this.missilRampes[i].Animate(deltaTime);
		}
	...
	if (this.lazerniums != null && this.lazerniumEnabled)
	{
		for (var i = 0; i < this.lazerniums.length; i++) {
			this.lazerniums[i].Animate(deltaTime);
		}
	}
};


SoucoupeAmirale3D.prototype.IsEndPhase = function () {
	return (this.phase == 7);
};

SoucoupeAmirale3D.prototype.Draw = function (ctx) {
	if (this.valid == false) return;
	...
	this.rayonEnergy.Draw(ctx);

	if (this.lazerniums != null && this.lazerniumEnabled)
	{
		for (var i = 0; i < this.lazerniums.length; i++) {
			this.lazerniums[i].Draw(ctx);
		}
	}
...
SoucoupeAmirale3D.prototype.OnEnded = function () {
	if (this.IsValid() == false) 
	{
		this.rayonEnergy.Enable = false;
		this.rayonEnergy.ResetTiming ();
	}
};

SoucoupeAmirale3D.prototype.IsStopMissil = function () {
	return true;
};

SoucoupeAmirale3D.prototype.StopGame = function () {
	this.rayonEnergy.StopGame ();
}
...
function SortieNavette (ownerEntity) {

	Entity.call(this,ownerEntity);
	this.Enable = true;
	this.newNavetteTime = new Timer (0.4);
	
}

SortieNavette.prototype = Object.create(Entity.prototype);
SortieNavette.prototype.constructor = SortieNavette;


SortieNavette.prototype.Animate = function (deltaTime) {

	if (this.Enable == false) return;

	if (this.newNavetteTime.Process (deltaTime) == true)
	{
		var navposx = this.OwnerEntity.posx + 187;
		var navposy = this.OwnerEntity.posy + 780;
		Sequencer.AddNavetteFromPosition (navposx,navposy, 4,1,randomUniform (1,5));

		navposx = this.OwnerEntity.posx + 773;
		Sequencer.AddNavetteFromPosition (navposx,navposy, 4,1,randomUniform (1,5));
		
		loader.PlaySound ('EnvoieNavette',0.4);

		this.newNavetteTime.Reset();
	}
};
...
function RayonEnergy(ParentEntity, relposx, relposy, LazerName) {
	GinginLazernium.call(this, ParentEntity, LazerName);
	...
	this.RayonTrack = function (deltaTime) {

		var start = this.GetLazerniumStartPos();
		var rottime = this.speedRot * deltaTime * Math.PI / 180;

		var vecx = this.Target.posx + this.Target.model.img.width / 2 - start.x;
		var vecy = this.Target.posy + this.Target.model.img.height / 2 - start.y;

		this.tmpLazerHeight = Math.sqrt(vecx * vecx + vecy * vecy);
		vecx /= this.tmpLazerHeight;
		vecy /= this.tmpLazerHeight;
	...
	this.NewMissil = function () {
		var missilmodel = loader.GetModel('RayonEnergyBall');
		var missil = new RayonEnergyBall();
		var start = this.GetLazerniumStartPos();
		missil.SetModel(missilmodel);
		missil.speed = missilmodel.speed;
		missil.timeLifeMax = 4;
		missil.Init (start.x, start.y, Goldo);

	
		missil.SetFireDirection(
			start.x,
			start.y,
			this.trackerdx, this.trackerdy);



		loader.PlaySound ('EnergyBall',1);
		missilList.AddObject(missil);
	};
}
...
RayonEnergy.prototype = Object.create(GinginLazernium.prototype);
RayonEnergy.prototype.constructor = RayonEnergy;

RayonEnergy.prototype.GetLazerniumStartPos = function () {
	return new Vector(this.Parent.posx + this.relposx, this.Parent.posy + this.relposy);
}

RayonEnergy.prototype.ResetTiming = function () {
...
RayonEnergy.prototype.StopGame = function () {
	this.ResetTiming();
};

RayonEnergy.prototype.Init = function () {

	this.Target = Goldo;
	this.Audio = loader.GetSoundByName ('LaserBeam');
	this.speedRot = 60;

	// this.fireDuration = this.Audio.duration; // Longuer du MP3 du lazer	
	this.fireDuration = randomUniform (this.Audio.duration / 2, this.Audio.duration + 1);
};

RayonEnergy.prototype.Animate = function (deltaTime) {
	...
	if (this.InFire == true)
	{
		...
		this.RayonTrack(deltaTime);
		
		if (this.fireTime > this.fireDuration)
		{
			this.fireTime = 0;
			this.InFire = false;
			this.nexfireTime = 0;
	...
	{
		this.nexfireTime += deltaTime;
		if (this.nexfireTime > 2) 
		{
			this.RayonTrack(deltaTime);
			this.InFire = true;
			this.fireDuration = randomUniform (2, this.Audio.duration + 1);
			this.AudioNode = loader.PlaySoundAudio (this.Audio,0.8);
		}
	}
}

RayonEnergy.prototype.Draw = function (ctx) {
	...
	ctx.drawImage(
		this.model.img, 0, 0,
		this.model.img.width, this.model.img.height, 0, 0
		, this.model.img.width, this.tmpLazerHeight);
	ctx.restore();
}
...
RayonEnergyBall.prototype = Object.create(Entity.prototype);
RayonEnergyBall.prototype.constructor = RayonEnergyBall;

RayonEnergyBall.prototype.Init = function (x, y, target) {

	this.posx = x - this.model.img.width / 2;
	this.posy = y - this.model.img.height / 2;

	// this.dx = target.posx + target.model.img.width / 2 - x;
	// this.dy = target.posy + target.model.img.height / 2 - y;
...
RayonEnergyBall.prototype.SetModel = function (model) {
	this.model = model;
	this.vie = model.vie;
	this.SetMinusRadius();
...
RayonEnergyBall.prototype.Animate = function (deltaTime) {
...
RayonEnergyBall.prototype.DrawShadow = function (ctx) {
};

RayonEnergyBall.prototype.Draw = function (ctx) {
	if (this.valid == false) return;
...
RayonEnergyBall.prototype.Collide = function (entity) {

	if (this.IsValid() == false) return false;

	// Test only this entity
	if (entity != Goldo
			&& entity != Goldo.Alcorak
			&& entity != Goldo.Fossoirak
			&& entity != Goldo.Venusiak
			&& entity != Goldo.OVTerre) return false;

	var x = this.posx + this.model.img.width / 2;
	var y = this.posy + this.model.img.height / 2;
	
	var centerx = entity.posx + entity.model.img.width / 2;
	var centery = entity.posy + entity.model.img.height / 2;

	if (CircleVsCircle (x,y,this.radius,centerx, centery, entity.radius))
	{
		// Indiquer à Goldorak qu'il ne peut plus tirer
		entity.ApplyEffect(0);
		this.Hit();
		this.SetNoValid();
		return true;
	}
...
// defaultDir est de type Vector
function LazerRampe(ParentEntity, relposx, relposy, defaultDir) {
	FireEntity.call(this, ParentEntity);
	this.relposx = relposx;
	this.relposy = relposy;
	this.defaultDir = (defaultDir == undefined) ? null : defaultDir;

	this.timeFire = new Timer(randomUniform(1000, 5000) / 1000);
	...
	this.Fire = function () {
		...
		var missilposy = this.Parent.posy + this.relposy;
		
		
		if (missilposy > AppViewModel.canvasize.height && defaultDir.y > 0) return;
		if (missilposy < 0 && defaultDir.y < 0) return;
		
		var targetposy = Goldo.posy + Goldo.model.img.height / 2;
		if (missilposy < targetposy && defaultDir.y < 0) return;
		if (missilposy > targetposy && defaultDir.y > 0) return;
		
		var missilModel = loader.GetModel('LazerAmirale3D');

		var missil = new LazerDirect (this, this.Parent.posx + this.relposx, missilposy,'LazerAmirale3D');
		missil.speed = (this.preferedMissilSpeed != 0) ? this.preferedMissilSpeed : missilModel.speed;

		missil.Init();
		missilList.AddObject(missil);

		loader.PlaySound('LazerDirect', 0.6);
	};
...
LazerRampe.prototype = Object.create(FireEntity.prototype);
LazerRampe.prototype.constructor = LazerRampe;

LazerRampe.prototype.Animate = function (deltaTime) {
	this.timeFire.Process(deltaTime);
	if (this.timeFire.IsOver()) {
		this.timeFire.SetTimeMax ((this.preferedFireTimeIteration != 0) ? this.preferedFireTimeIteration : randomUniform(1000, 5000) / 1000, true);
		this.Fire();
	}
};
...
// function LazerDirect(ParentEntity, relposx, relposy, LazerName) {
function LazerDirect(ParentEntity, posx,posy, LazerName) {
	GinginLazernium.call(this, ParentEntity, LazerName);
	...
	this.RayonTrack = function () {
		var start = this.GetLazerniumStartPos();

		this.dx = this.Target.posx + this.Target.model.img.width / 2 - start.x;
		this.dy = this.Target.posy + this.Target.model.img.height / 2 - start.y;

		var len = Math.sqrt(this.dx*this.dx +this.dy*this.dy);
		this.dx /= len;
		this.dy /= len;

		this.angtracker = Math.atan2(this.dy, this.dx) - Math.PI / 2;

		this.trackerdx = -Math.sin(this.angtracker);
		this.trackerdy = Math.cos(this.angtracker);
		
		// this.rect = new RectSize(0,0,this.dx * this.model.img.height,this.dy * this.model.img.height);
		this.rect = new RectSize(0,0,this.dx,this.dy);
	...
	this.CollideEntity = function (Target) {
		...
		if (SegmentVsCircle(start.x, start.y, this.posx, this.posy, cx, cy, Target.radius) == true) {
			return true;
		}
...
LazerDirect.prototype = Object.create(GinginLazernium.prototype);
LazerDirect.prototype.constructor = LazerDirect;

LazerDirect.prototype.GetLazerniumStartPos = function () {
	return new Vector(this.prevposx,this.prevposy);
}

LazerDirect.prototype.Init = function () {

	this.fireDuration = 5;
	this.RayonTrack();
};

LazerDirect.prototype.Animate = function (deltaTime) {
	...
	if (this.tmpLazerHeight == this.model.img.height)
	{
		this.prevposx = this.posx - this.dx * this.model.img.height;
		this.prevposy = this.posy - this.dy * this.model.img.height;
	}
	else
	{
		var ddx = this.posx - this.prevposx;
		var ddy = this.posy - this.prevposy;
		var len = Math.sqrt(ddx * ddx + ddy * ddy);
		if (len >= this.model.img.height)
		{
			this.tmpLazerHeight = this.model.img.height;
		}
		else this.tmpLazerHeight = len;
		this.rect = new RectSize(0,0,this.dx*this.tmpLazerHeight,this.dy*this.tmpLazerHeight);
	}
	...
	// if (this.prevposy > AppViewModel.canvasize.height) {
		// this.SetNoValid();
	// }
	
	if (this.prevposx < 0
	|| this.prevposx > AppViewModel.canvasize.width
	|| this.prevposy < 0
	|| this.prevposy > AppViewModel.canvasize.height)
	{
		this.SetNoValid();
	}
...
LazerDirect.prototype.Draw = function (ctx) {

	if (this.valid == false) return;

	var startx = this.prevposx - this.trackerdy * this.model.img.width / 2;
	var starty = this.prevposy + this.trackerdx * this.model.img.width / 2;
...
LazerDirect.prototype.GetProcessCollisionBBox = function () {
	return new BBox(this.prevposx,this.prevposy,this.posx,this.posy);
}

LazerDirect.prototype.IsEnableCollideSameCamp = function () {
	return false;
}

LazerDirect.prototype.Collide = function (entity) {

	if (entity.model.camp == this.model.camp) return false;
	return this.CollideEntity(entity);
};
...
function Megavolt(ParentEntity) {
	...
	this.resetHeightOnCollision = false;
...
Megavolt.prototype.Animate = function (deltaTime) {
	...
	this.timeLife += deltaTime;
	if (this.countFire == 0 && this.timeLife > this.timeLifeMax) {
		this.Enable = false;
		this.InFire = false;
		return;
	}
	...
	if (this.InFire == true) {
		...
		var list = Scape.GetNodeRectList(bbox.x1,bbox.y1,bbox.x2,bbox.y2);
			...
			if (scapeNode.obj.EntityList != null) {
				...
				while (entityNode != null) {
					...
					if (entity.model.camp != AppViewModel.campGoldo) {
						if (CollideRect(entity, bbox.x1,bbox.y1,bbox.x2,bbox.y2) == true) {
							entity.DecreaseShieldForce(this.model.degat);
							if (this.resetHeightOnCollision) this.tmpLazerHeight = 0;
							if (entity.ShieldForce <= 0) {
								entityList.AddObject(entity);

								if (this.hasSparkBlue == true)
									NewSparkBluePos(this.Parent.posx + this.Parent.model.img.width / 2, bbox.y1);
								else
									NewExplosionPos(this.Parent.posx + this.Parent.model.img.width / 2, bbox.y1);
...
Megavolt.prototype.GetProcessCollisionBBox = function () {
	...
	var startx = this.widthAdjust + this.Parent.posx + this.Parent.model.img.width / 2 - this.model.img.width / 2;
	var starty = this.Parent.posy + this.heigthAdjust - this.tmpLazerHeight;
	
	var endx = startx + this.model.img.width;
	var endy = starty + this.tmpLazerHeight;
...
function Mortanium(ParentEntity,type) {
	FireEntity.call(this, ParentEntity);

	this.typeMortanium = type;
	this.tmpLazerHeight = 0;
	this.heigthAdjust = 0;
	this.widthAdjust = 0;
	this.timeLife = 0;
	...
	this.countFire = 0;
	this.hasSparkBlue = true;
	this.MortaniumMods = null;
	this.MortaniumImage = new Indexer(1, 3);
	this.timeFireImageEffect = new Timer(0.12);
	this.resetHeightOnCollision = true;
	...
	this.EnableFire = function () {
		this.Enable = true;
		this.timeLife = 0;
	};
	
	
	this.GetMortaniumRelPos = function (type) {

		var relposx = 0;
		var relposy = 0;
		switch (type) {
			case 1: relposx = 18; relposy = 48; break;
			case 2: relposx = 133; relposy = 48; break;
		}
	...
	this.GetMortaniumHeaderRelPos = function (type) {

		var relposx = 0;
		var relposy = 0;
		switch (type) {
			case 1: relposx = 18; relposy = 40; break;
			case 2: relposx = 133; relposy = 40; break;
		}
...
// Episode de l'araignée Meka bombe
// Bruitage de goldorak qui marche sans musique

// Episode 42
// Victorang 17:33 environ
// Mortanium 18:40 environ


// Episode 74
// Pulvonium 2:43 environ


Mortanium.prototype = Object.create(FireEntity.prototype);
Mortanium.prototype.constructor = Mortanium;

Mortanium.prototype.Init = function () {

	this.HeaderMod = loader.GetModel('MortaniumHeader');
	
	var rel = this.GetMortaniumRelPos (this.typeMortanium);
	this.widthAdjust = rel.x;
	this.heigthAdjust = rel.y;


	this.MortaniumMods = new Array();
	for (var i = this.MortaniumImage.minValue; i <= this.MortaniumImage.maxValue; i++) {
		this.MortaniumMods[i] = loader.GetModel('MortaniumMap' + i);
	}
	
	this.model = this.MortaniumMods[this.MortaniumImage.curValue];
	this.segray = this.model.img.width;
	this.InFire = false;
};

/*
Mortanium.prototype.Animate = function (deltaTime) {

	if (this.Enable == false) return;
	
	this.timeFireImageEffect.Process(deltaTime);
	if (this.timeFireImageEffect.IsOver())
	{
		this.timeFireImageEffect.Reset();
		this.MortaniumImage.Inc();
		
		this.model = this.MortaniumMods[this.MortaniumImage.curValue];
	}

	Megavolt.prototype.Animate.call(this, deltaTime);
};
*/
			...
			if (scapeNode.obj.EntityList != null) {
				...
				while (entityNode != null) {
					...
					if (entity.model.camp != AppViewModel.campGoldo) {
						...
						var col;
						if (entity.model.quadNode != null)
						{
							col = CollideRect(entity, bbox.x1,bbox.y1,bbox.x2,bbox.y2);
						}
						else
						{
							var xcol = (bbox.x1 + bbox.x2) / 2;
							col = SegmentVsCircle(xcol,bbox.y1,xcol,bbox.y2,
								entity.posx + entity.model.img.width/2,
								entity.posy + entity.model.img.height/2,
								entity.radius);
						}
...
Mortanium.prototype.GetProcessCollisionBBox = function () {

	var startx = this.widthAdjust + this.Parent.posx  - this.model.img.width / 2;
	var starty = this.Parent.posy + this.heigthAdjust - this.tmpLazerHeight;
...
Mortanium.prototype.Draw = function (ctx) {
	...
	// 3 36
	// 118 36
	var relHead = this.GetMortaniumHeaderRelPos (this.typeMortanium);

	ctx.drawImage(this.HeaderMod.img, this.Parent.posx + relHead.x - this.HeaderMod.img.width / 2, this.Parent.posy + relHead.y);
	...
	ctx.drawImage(
		...
		this.Parent.posx + this.widthAdjust - this.model.img.width / 2,
...
Mortanium.prototype.Hit = function () {
};
...
function CorneAuFulgure(ParentEntity) {
	...
	this.timeUse = GoldoEnum.cornoFulgureTimeRecharge;
...
CorneAuFulgure.prototype.Init = function () {
	...
	// Le premier cornofulgure doit etre dispo tout de suite
	this.timeUse = GoldoEnum.cornoFulgureTimeRecharge;
};
...
function Golgoth27Tentacule(ParentEntity, tentaculeType) {
	...
	this.StartRotate = function () {
		...
		this.rotateTime = randomUniform(3, 10);
		this.tentaculeLen = this.tentaculeLenMin;
		this.tentaculeGrow = 1;
		this.angRotateSens = (randomUniform(1, 11) < 6) ? -1 : 1;
	}
...
Companion.prototype.ApplyEffect = function (type) {
	Goldo.ApplyEffect(type);
};
...
function Alcorak() {
	...
	this.NewMissilGamma = function (type) {
		...
		switch (type) {
			case 1:
				missil.posx = this.posx + (this.model.img.width / 2) - 15 - modelMissilGamma.img.width / 2;
	...
	this.WinMortanium = function () {
		...
		for (var i = 0; i <= 1; i++) {
			this.fireMortaniums[i].EnableFire();
		}
	};	
}
...
Alcorak.prototype.Init = function () {
	...
	this.fireMortaniums = new Array();
	for (var i = 0; i <= 1; i++) {
		
		this.fireMortaniums[i] = new Mortanium(this,i+1);
		this.fireMortaniums[i].Init();
	}
};
...
Alcorak.prototype.Fire = function () {
	...
	for (var i = 0; i <= 1; i++) {
		if (this.fireMortaniums[i].Enable) this.fireMortaniums[i].Fire();
	}
}
...
Alcorak.prototype.IncreaseLevel = function () {

	this.level++;

	if (this.level == 4) {
		loader.PlaySound('Victorang', 1);
	}
...
Alcorak.prototype.Animate = function (deltaTime) {
	if (this.valid == false) return;

	Companion.prototype.Animate.call(this, deltaTime);

	for (var i = 0; i <= 1; i++) {
		if (this.fireMortaniums[i].Enable) this.fireMortaniums[i].Animate (deltaTime);
	}
};



Alcorak.prototype.Draw = function (ctx) {
	if (this.valid == false) return;

	Companion.prototype.Draw.call(this, ctx);

	for (var i = 0; i <= 1; i++) {
		if (this.fireMortaniums[i].Enable) this.fireMortaniums[i].Draw(ctx)
	}
};
...
function Fossoirak() {
	...
	this.NewMissilSigma = function (type) {
		...
		y = missil.model.img.height;
		switch (type) {
			// case 1: x = 7; y = 24; break;
			case 1: x = this.model.img.width / 2 - 21 - missil.model.img.width / 2;break;

			// case 2: x = 58; y = 53; break;
			case 2: x = this.model.img.width / 2 + 21 - missil.model.img.width / 2;break;
		}
	...
	this.NewPyroBombe = function () {
		...
		pyro.posx = this.posx + this.model.img.width / 2 - pyro.model.img.width / 2;
	...
	this.NewMissilGamma = function (type) {
		...
		switch (type) {
			...
			// case 1: x = 7; y = 24; break;
			case 1: x = this.model.img.width / 2 - 48; y = 24; break;

			// case 2: x = 58; y = 53; break;
			case 2: x = this.model.img.width / 2 + 48; y = 53; break;
			
			
			case 3: x = this.model.img.width / 2; y = 50; break;
		}
...
function Venusiak() {
	...
	this.NewMissilGamma = function (type) {
		...
		switch (type) {
			...
			case 1: x = this.model.img.width / 2 - 50; y = 42; break;
			
			
			
			// case 2: x = 64; y = 7; break;
			case 2: x = this.model.img.width / 2 + 50; y = 42; break;
...
Venusiak.prototype.IncreaseLevel = function () {
	...
	switch (this.level) {
		case 3: loader.PlaySound('DeltaLame', 1); break;
	}
...
function MegaMach(parentEntity, timeLifeMax, modelName, nbcol, nbrow) {
	...
	this.Accumulate = function () {
		...
		{
			this.nbAccumulate ++;
			if (this.nbAccumulate == 3) loader.PlaySound('UltraCalorie', 1);
			else if (this.nbAccumulate > 3) 
			{
				NewCrosseExplosionPos(Goldo.posx + Goldo.model.img.width / 2, Goldo.posy + Goldo.model.img.height / 2);
				this.nbAccumulate = 0;
				Goldo.vie = 0;
			}
		}
...
function Road07Item(sequence,roadType,posx, posy) {
	...
	this.GetModelItem = function (type) {
		var mod = null;
		switch(type)
		{
			case RoadTypeMod.left:mod = loader.GetModel('Road07TurnLeft');break;
			case RoadTypeMod.forw:mod = loader.GetModel('Road07Forward');break;
			case RoadTypeMod.right:mod = loader.GetModel('Road07TurnRight');break;
			case RoadTypeMod.bleft:mod = loader.GetModel('Road07BordLeft');break;
			case RoadTypeMod.bforw:mod = loader.GetModel('Road07BordForward');break;
			case RoadTypeMod.bright:mod = loader.GetModel('Road07BordRight');break;
			case RoadTypeMod.end:mod = loader.GetModel('EndRoad07');break;
		}
		return mod;
	};	

	this.GetRoadModel = function (roadType) {
		var mod = null;
		switch(roadType)
		{
			case RoadType.left:mod = this.GetModelItem(RoadTypeMod.left);break;
			case RoadType.forw:mod = this.GetModelItem(RoadTypeMod.forw);break;
			case RoadType.right:mod = this.GetModelItem(RoadTypeMod.right);break;
			case RoadType.end:mod = this.GetModelItem(RoadTypeMod.end);break;
		}
		return mod;
	};	

	this.GetBorderModel = function (roadType) {
		var mod = null;
		switch(roadType)
		{
			case RoadType.left:mod = this.GetModelItem(RoadTypeMod.bleft);break;
			case RoadType.forw:mod = this.GetModelItem(RoadTypeMod.bforw);break;
			case RoadType.right:mod = this.GetModelItem(RoadTypeMod.bright);break;
		}
		return mod;
	};	
}

Road07Item.prototype = Object.create(Entity.prototype);
Road07Item.prototype.constructor = Road07Item;



Road07Item.prototype.Draw = function (ctx) {
	if (this.valid == false) return;

	if (this.posx < -this.model.img.width
		|| this.posx > AppViewModel.canvasize.width
		|| this.posy < -this.model.img.height
		|| this.posy > AppViewModel.canvasize.height) return;
...
Road07Item.prototype.Animate = function (deltaTime) {
	if (this.valid == false) return;
	this.posy += scrollZone.Speed * deltaTime;
...
Road07Item.prototype.IsInvulnerable = function () {
	return true;
};

Road07Item.prototype.IsStopMissil = function () {
	return true;
};



function Road07Pass(sequence, roadType, posx, posy) {
	Road07Item.call(this, sequence, roadType, posx, posy);
}

Road07Pass.prototype = Object.create(Road07Item.prototype);
Road07Pass.prototype.constructor = Road07Pass;

Road07Pass.prototype.Init = function () {
	this.SetModel (this.GetRoadModel(this.roadType));
}

Road07Pass.prototype.Collide = function (entity) {
	
	if (entity != Goldo) return false;
	
	// On ne traite que Goldo
	if (Collide(this, Goldo) == false) return false;
	
	// Si ce portion de route n'a pas encore été en collision
	// On comptabilise ce portion de route et si c'est un virage on envoie un son
	if (this.collided == false) 
	{
		// AppViewModel.nbRoad07Passed ++;
		this.sequence.PassRoad(this);
		
		if (this.roadType == RoadType.left || this.roadType == RoadType.right || this.roadType == RoadType.end)
			loader.PlaySound('GoldoPassage01', 1);
	}
	this.collided = true;

	// On renvoie false pour indiquer que l'effet a été traité ici
	return false;
}


function Road07Block(sequence, roadType, posx, posy) {
	Road07Item.call(this, sequence, roadType, posx, posy, false);
}

Road07Block.prototype = Object.create(Road07Item.prototype);
Road07Block.prototype.constructor = Road07Block;

Road07Block.prototype.Init = function () {
	this.SetModel (this.GetBorderModel(this.roadType));
}

Road07Block.prototype.Collide = function (entity) {
	return false;
}
...
function Silo() {
	...
	this.siloIllumine = loader.GetModel('SiloIllumine');
	this.siloDraw = null;
	this.siloIllumineTime = 0;
...
Silo.prototype.Animate = function (deltaTime) {
	...
	this.siloIllumineTime += deltaTime;
	if (this.siloIllumineTime > 1.5)
	{
		this.siloDraw = (this.siloDraw == this.model) ? this.siloIllumine : this.model
		this.siloIllumineTime = 0;
	}
...
Silo.prototype.Draw = function (ctx) {
	...
	ctx.drawImage(this.insiMod.img, this.posx, this.posy);
	...
	ctx.drawImage(this.socMod.img, this.posx + this.model.img.width / 2 - this.socMod.img.width / 2, this.posy + 25);
	ctx.drawImage(this.siloDraw.img, this.posx, this.posy);
...
Silo.prototype.OnEnded = function () {
	...
	if (this.vie <= 0) {
		...
		decorManager.NewGroundDecor(
			DecorEnum.other, this.posx + destMod.img.width / 2 - 82 ,
			this.posy + destMod.img.height / 2 + 10, 'SiloDetruit');

		// ctx.drawImage(destMod.img, this.posx + this.model.img.width / 2 - destMod.img.width / 2, this.posy + 8);

			
	}
};

Silo.prototype.CanCollide = function () {
	return false;
};
...
// Classe d'objet de timer arrivée hyper super hyper super tard
// a terme je l'emploierai surement pour remplacer les varaible de gestion de temps
// un peu partout dans le code.
function Timer(timemax) {
	this.timeSpent = 0;
	this.timeMax = (timemax != undefined) ? timemax : 1;
	this.enable = true;
	...
	this.IsOver = function () {
		return (this.timeSpent >= this.timeMax) ? true : false;
	};

	this.ForceOver = function () {
		this.timeSpent = this.timeMax;
	};
	...
	this.IsHalfPassed = function () {
		return (this.timeSpent >= this.timeMax / 2) ? true : false;
	};

	this.IsQuarterPassed = function () {
		return (this.timeSpent >= this.timeMax / 4) ? true : false;
	};


	this.SetTimeMax = function (timemax, breset) {
		this.timeMax = (timemax != undefined) ? timemax : this.timeMax;
		if (breset != undefined && breset == true) this.Reset();
	};


}

// Called when timer has reached max time
Timer.prototype.OnOver = function () {
};
...
// Par defaut il est cyclique
// Classe d'objet qui permet de gérer des indexes ou des numéro
// par exemple on une série d'images animées indexées 0, 1, 2 et 3
// On utilise cet indexer pour incrémenter ou décrémenter l'index
// dans les limites de this.minValue et this.maxValue
function Indexer(minValue, maxValue,initialValue, cyclic) {
	this.minValue = minValue;
	this.maxValue = maxValue;
	this.initialValue = (initialValue != undefined) ? initialValue : this.minValue;
	this.curValue = this.initialValue;
	this.cyclic = (cyclic != undefined) ? cyclic : true;

	this.Inc = function () {
		this.curValue ++;
		if (this.curValue > this.maxValue)
		{
			this.curValue = (this.cyclic == true) ? this.minValue : this.maxValue;
		}
	...
	this.Dec = function () {
		...
		{
			this.curValue = (this.cyclic == true) ? this.maxValue : this.minValue;
		}
		return this.curValue;
	}
...
Blocker.prototype = Object.create(Entity.prototype);
Blocker.prototype.constructor = Blocker;

Blocker.prototype.Init = function () {

	this.SetModel(loader.GetModel('Blocker'));
	
	
	switch (this.type)
	{
		case BlockerEnum.left:
		this.ang = 0;
		this.tx = 1;
		this.dx = 1;
		this.dy = 0;
		this.minx = this.model.img.width/2;
		this.maxx = AppViewModel.canvasize.width / 2 - this.model.img.width*1.5;

		this.minx = 0;
		this.maxx = AppViewModel.canvasize.width / 2;

		break;
		
		case BlockerEnum.right:
		this.ang = Math.PI;
		this.tx = -1;
		this.dx = -1;
		this.dy = 0;
		this.minx = AppViewModel.canvasize.width / 2 + this.model.img.width * 1.5;
		this.maxx = AppViewModel.canvasize.width - this.model.img.width;
		
		this.minx = AppViewModel.canvasize.width / 2;
		this.maxx = AppViewModel.canvasize.width;

		break;
	}

	this.posx = randomUniform (this.minx,this.maxx);
	this.posy = -this.model.img.height;
	...
	this.SetMinusRadius();
		this.rect = new RectSize(
		-this.model.img.width / 2,
		-this.model.img.height / 2, this.model.img.width, this.model.img.height);
	
	var modl = loader.GetModel ('BlockerRayon');
	this.Lazer01 = new BlockerLazernium(this, this.model.img.width / 2 - 20, - modl.img.height / 2, false, 'BlockerRayon');
	this.Lazer01.Init ();
	this.Lazer02 = new BlockerLazernium(this, this.model.img.width / 2 - 20, modl.img.height / 2, false, 'BlockerRayon');
	this.Lazer02.Init ();
	

	var modrail = loader.GetModel('BlockerRail');
	var nbrail = 1 + (this.maxx - this.minx) / modrail.img.width;
	
	for (var i = 0; i < nbrail ; i++)
	{
		decorManager.NewGroundDecor(DecorEnum.other,
		this.minx + (i * modrail.img.width),
		this.posy + this.model.img.height / 2, 'BlockerRail');
	}
	
};


Blocker.prototype.Animate = function (deltaTime) {
	if (this.valid == false) return;
	this.posy += scrollZone.Speed * deltaTime;
	...
	this.posx += this.speed * this.tx * deltaTime;
	...
	if (this.Lazer01 != null) this.Lazer01.Animate (deltaTime);
	if (this.Lazer02 != null) this.Lazer02.Animate (deltaTime);

	
};


Blocker.prototype.Draw = function (ctx) {
	if (this.valid == false) return;

	if (this.Lazer01 != null) this.Lazer01.Draw (ctx);
	if (this.Lazer02 != null) this.Lazer02.Draw (ctx);
...
Blocker.prototype.IsStopMissil = function () {
	return true;
};

Blocker.prototype.IsInvulnerable = function () {
	return false;
};

Blocker.prototype.OnEnded = function () {
	if (this.IsValid() == false && this.vie <= 0) {
		NewCrosseExplosionPos(this.posx + this.model.img.width / 2, this.posy + this.model.img.height / 2);
	}
};

Blocker.prototype.CanCollide = function (entity) {
	return false;
};



Blocker.prototype.Collide = function (entity) {

	// On n'accepte que ces entités
	if (entity != Goldo && entity != Goldo.Alcorak && entity != Goldo.Fossoirak && entity != Goldo.Venusiak && entity != Goldo.OVTerre)
		return false;
	...
	// Un collision avec une entité on explose
	this.vie = 0;
...
// posxrel, posyrel son des position relative par rapport au centre de l'image de on parent
function BlockerLazernium(ParentEntity, posxrel, posyrel, TranslationEffect, LazerName) {
	FireEntity.call(this, ParentEntity);

	this.LazerName = LazerName;
	this.TranslationEffect = TranslationEffect;

	// this.Enable = false;
	this.tmpLazerHeight = 0;
	this.posxRel = posxrel;
	this.posyRel = posyrel;

	this.posxRelTime = posxrel;
	this.posyRelTime = posyrel;

	this.enableExplosion = true;
	
	this.spentTime = 0;
	this.nextTimeFire = randomUniform (500,1500) / 1000;
	this.InFire = false;
	...
	this.CollideEntity = function (Target) {
		...
		var x1 = this.posxRelTime;
		var y1 = this.posyRelTime;
...
BlockerLazernium.prototype = Object.create(FireEntity.prototype);
BlockerLazernium.prototype.constructor = BlockerLazernium;


BlockerLazernium.prototype.Init = function () {

	this.SetModel(loader.GetModel(this.LazerName));
	this.segray = this.model.img.width;
};

BlockerLazernium.prototype.Animate = function (deltaTime) {

	if (this.Enable == false) return;


	var mat = new Mat2D(this.Parent.dx, this.Parent.dy);

	vec1 = mat.Mul(this.posxRel,this.posyRel);
	vec1.x += this.Parent.posx + this.Parent.model.img.width / 2;
	vec1.y += this.Parent.posy + this.Parent.model.img.height / 2;

	this.posxRelTime = vec1.x;
	this.posyRelTime = vec1.y;
	
	if (this.InFire == false) {
		
		this.spentTime += deltaTime;
		if (this.spentTime >= this.nextTimeFire)
		{
			this.InFire = true;
			this.tmpLazerHeight = 0;
			this.PlaySound();
	...
	else
	{
		...
		if (this.tmpLazerHeight > this.model.img.width) this.tmpLazerHeight = this.model.img.width;
		this.ProcessCollision();

		if (this.tmpLazerHeight >= this.model.img.width) {
			this.ResetTiming();
		}
...
BlockerLazernium.prototype.ResetTiming = function () {
	this.tmpLazerHeight = 0;
	this.InFire = false;
	this.nextTimeFire = randomUniform (1500,3000) / 1000;
	this.spentTime = 0;
}



BlockerLazernium.prototype.Draw = function (ctx) {

	/*
	DrawCircle (ctx,this.posxRelTime,this.posyRelTime,10);
	
	DrawLine (ctx,this.posxRelTime,this.posyRelTime,
	this.posxRelTime + this.Parent.dx * this.tmpLazerHeight
	,this.posyRelTime + this.Parent.dy * this.tmpLazerHeight,'#AAAAFF');
	*/
	
	ctx.save();
	ctx.translate(this.posxRelTime, this.posyRelTime);

	ctx.rotate(this.Parent.ang);
...
BlockerLazernium.prototype.Hit = function () {
};

BlockerLazernium.prototype.PlaySound = function () {

	loader.PlaySound('Lazernium', 1);
};

BlockerLazernium.prototype.GetProcessCollisionBBox = function () {

	var startx = this.posxRelTime;
	var starty = this.posyRelTime;
...
BlockerLazernium.prototype.IsEnableCollideSameCamp = function () {
